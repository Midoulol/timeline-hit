...
function serveMedia(req: Request, url: URL): Response {
  const raw = decodeURIComponent(url.searchParams.get("p") ?? "");
  ...
  return new Response(file as BodyInit, {
    status: 200,
    headers: { ...CORS, "Content-Type": "video/mp4", "Accept-Ranges": "bytes" },
  });
}
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      try {
        ...
        if (url.pathname === "/api/pick-subtitle") return json(await pickSubtitle(store));
...
async function pickVideo(store: AppStore) {
  const path = firstExisting(DEFAULT_VIDEO);
